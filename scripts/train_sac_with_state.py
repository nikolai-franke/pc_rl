...
def build(config: DictConfig):
    ...
    observation: ArrayDict[Array] = ArrayDict()

    observation["point_cloud"] = dict_map(
        Array.from_numpy,
        metadata.example_obs["point_cloud"],
        batch_shape=tuple(batch_spec),
        max_mean_num_elem=obs_space["point_cloud"].shape[0],
        kind="jagged",
        storage=storage,
        padding=1,
        full_size=replay_length,
    ...
    observation["state"] = dict_map(
        Array.from_numpy,
        metadata.example_obs["state"],
        batch_shape=tuple(batch_spec),
        storage=storage,
        padding=1,
        full_size=replay_length,
    )
    sample_tree["observation"] = observation
    ...
    mlp_input_size = finetune_encoder.dim + obs_space["state"].shape[0]
