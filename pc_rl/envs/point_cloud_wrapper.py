...
class PointCloudObservationWrapper(gym.ObservationWrapper):
    ...
    def reset(self, **kwargs):
        ...
        self.position_containers = self.env.scene_creation_result["pointcloud_objects"][
            "position_containers"
        ]
        self.triangle_containers = self.env.scene_creation_result["pointcloud_objects"][
            "triangle_containers"
        ]
        ...
        if self.camera_configs is None:
            ...
            if isinstance(
                self.env.scene_creation_result["camera"],
                self.env.camera_templates.Camera,
            ):
                self.camera_object = self.env.scene_creation_result[
                    "camera"
                ].sofa_object
            else:
                self.camera_object = self.env.scene_creation_result["camera"]
            ...
            for camera in self.camera_configs:
                ...
                for key in required_keys:
                    if key not in camera:
                        raise KeyError(
                            f"Could not find key {key} in configuration for camera {camera}."
                        )
    ...
    def create_open3d_scene_from_containers(self) -> Type[o3d.t.geometry.PointCloud]:
        ...
        for position_container, triangle_container in zip(
            self.position_containers, self.triangle_containers
        ):
            mesh = o3d.t.geometry.TriangleMesh()
            mesh.vertex.positions = o3d.core.Tensor(
                position_container.position.array(), o3d.core.float32
            )
            mesh.triangle.indices = o3d.core.Tensor(
                triangle_container.triangles.array(), o3d.core.int32
            )
    ...
    def create_point_cloud_from_scene_objects(self) -> np.ndarray:
        ...
        for rays in ray_list:
            ...
            if self.append_object_id:
                ids = (
                    ans["geometry_ids"][hit]
                    .reshape((-1, 1))
                    .to(dtype=o3d.core.Dtype.Float32)
                )
