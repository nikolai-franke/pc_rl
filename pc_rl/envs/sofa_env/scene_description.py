...
from sofa_env.scenes.rope_threading.sofa_objects.gripper import (
    GRIPPER_PLUGIN_LIST, ArticulatedGripper)
from sofa_env.sofa_templates.camera import CAMERA_PLUGIN_LIST, Camera
from sofa_env.sofa_templates.collision import (COLLISION_PLUGIN_LIST,
                                               CollisionModelType,
                                               add_collision_model)
from sofa_env.sofa_templates.rigid import (RIGID_PLUGIN_LIST,
                                           MechanicalBinding, RigidObject)
from sofa_env.sofa_templates.rope import (ROPE_PLUGIN_LIST, Rope,
                                          poses_for_circular_rope)
from sofa_env.sofa_templates.scene_header import (SCENE_HEADER_PLUGIN_LIST,
                                                  ...
                                                  IntersectionMethod,
                                                  add_scene_header)
from sofa_env.sofa_templates.visual import (VISUAL_PLUGIN_LIST,
                                            add_visual_model, set_color)
from sofa_env.utils.camera import determine_look_at
from sofa_env.utils.math_helper import (euler_to_rotation_matrix,
                                        rotation_matrix_to_quaternion)
from sofa_env.utils.pivot_transform import \
    sofa_orientation_to_camera_orientation
...
INSTRUMENT_MESH_DIR = Path("/home/nikolai/repos/sofa_env_devel/assets/meshes/instruments")
TEXTURE_DIR = Path("/home/nikolai/repos/sofa_env_devel/assets/textures")


class Peg(RigidObject):
    def set_color(self, new_color: Tuple[int, int, int]):
        set_color(
            self.visual_model_node.OglModel,
            color=tuple(color / 255 for color in new_color),
        )
...
def createScene(
    ...
    add_scene_header(
        ...
        visual_style_flags=VISUAL_STYLES["debug"]
        if debug_rendering
        else VISUAL_STYLES["normal"],
    ...
    camera_orientation = sofa_orientation_to_camera_orientation(
        rotation_matrix_to_quaternion(
            euler_to_rotation_matrix(np.array([45.0, 0.0, 180.0]))
        )
    )
    ...
    board_visual_function = partial(
        add_visual_model, texture_file_path=str(board_texture_path)
    )
    RigidObject(
    ...
    rope_poses = poses_for_circular_rope(
        radius=radius,
        num_points=num_points,
        start_position=rope_start_position,
        euler_angle_rotation=rope_euler_angles,
    )
    ...
    gripper = scene_node.addObject(
        ArticulatedGripper(
            ...
            visual_mesh_paths_jaws=[
                INSTRUMENT_MESH_DIR / "forceps_jaw_left.stl",
                INSTRUMENT_MESH_DIR / "forceps_jaw_right.stl",
            ],
