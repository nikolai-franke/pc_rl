...
class ReachEnv(SofaEnv):
    ...
    def __init__(
        ...
        if observation_type == ObservationType.STATE:
            ...
            self.observation_space = spaces.Box(
                low=-1.0, high=1.0, shape=(dim_states,), dtype=np.float32
            )
    ...
    def _init_sim(self):
        ...
        self.end_effector: EndEffector = self.scene_creation_result[
            "interactive_objects"
        ]["end_effector"]
        self._visual_target: ControllableRigidObject = self.scene_creation_result[
            "interactive_objects"
        ]["visual_target"]
        ...
        self._distance_normalization_factor = 1.0 / np.linalg.norm(
            self._workspace["high"][:3] - self._workspace["low"][:3]
        )
    ...
    def _get_reward_features(self, previous_reward_features: dict) -> dict:
        ...
        reward_features["distance_to_target"] = np.linalg.norm(
            current_position - target_position
        )
        reward_features["delta_distance_to_target"] = (
            reward_features["distance_to_target"]
            - previous_reward_features["distance_to_target"]
        )

        if reward_features["delta_distance_to_target"] == 0:
            reward_features["cos_reward"] = 0
        else:
            cosine_similarity = np.abs(
                np.dot(
                    (current_position - self.previous_position),
                    (target_position - self.previous_position),
                )
            ) / (
                np.linalg.norm(current_position - self.previous_position)
                * np.linalg.norm(target_position - self.previous_position)
            )
            reward_features["cos_reward"] = (
                reward_features["delta_distance_to_target"] * cosine_similarity
            )
        ...
        reward_features["workspace_violation"] = float(
            self._last_action_violated_workspace
        )

        reward_features["successful_task"] = float(
            reward_features["distance_to_target"] <= self._distance_to_target_threshold
        )
    ...
    def _normalize_position(self, position: np.ndarray) -> np.ndarray:
        ...
        return (
            2
            * (position - self._workspace["low"][:3])
            / (self._workspace["high"][:3] - self._workspace["low"][:3])
            - 1
        )
    ...
    def _get_observation(
        ...
        if self._observation_type == ObservationType.RGB:
            if self._observe_target_position:
                observation = {
                    ...
                    "target_position": self._normalize_position(
                        self._visual_target.get_pose()[:3]
                    ),
                }
            else:
            ...
            if self._observe_target_position:
                ...
                observation["target_position"][:] = self._normalize_position(
                    self._visual_target.get_pose()[:3]
                )
            else:
                observation = self.observation_space.sample()
            ...
            if self._observe_target_position:
                observation[3:] = self._normalize_position(
                    self._visual_target.get_pose()[:3]
                )
    ...
    def reset(
        ...
        while not new_target_position_found:
            target_pose = self.rng.uniform(
                self._workspace["low"], self._workspace["high"]
            )
            initial_distance = np.linalg.norm(
                self.end_effector.initial_pose[:3] - target_pose[:3]
            )
            new_target_position_found = (
                initial_distance >= self._minimum_distance_to_target_at_start
            )
        ...
        self.reward_features = {}
        self.reward_features["distance_to_target"] = np.linalg.norm(
            self.end_effector.get_pose()[:3] - self._visual_target.get_pose()[:3]
        )
        self.previous_position = self.end_effector.get_pose()[:3].copy()
