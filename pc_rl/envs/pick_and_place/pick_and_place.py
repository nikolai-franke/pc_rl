...
from sofa_env.scenes.rope_threading.sofa_objects.gripper import \
    ArticulatedGripper
...
class PickAndPlaceEnv(SofaEnv):
    ...
    def __init__(
        ...
        maximum_state_velocity: Union[np.ndarray, float] = np.array(
            [20.0, 20.0, 20.0, 20.0, 5.0]
        ),
        discrete_action_magnitude: Union[np.ndarray, float] = np.array(
            [15.0, 15.0, 15.0, 10.0, 2.0]
        ),
        ...
        if only_learn_pick and start_grasped:
            raise Exception(
                f"Environment was asked to skip the pick phase ({start_grasped=}) and also only learn to pick ({only_learn_pick=})."
            )
        ...
        if action_type == ActionType.CONTINUOUS:
            self.action_space = spaces.Box(
                low=-1.0, high=1.0, shape=(action_dimensionality,), dtype=np.float32
            )
            ...
            for i in range(action_dimensionality * 2):
                ...
                step_size = (
                    discrete_action_magnitude
                    if isinstance(discrete_action_magnitude, float)
                    else discrete_action_magnitude[int(i / 2)]
                )
        ...
        self.torus_tracking_point_indices: Union[
            np.ndarray, None
        ]  # set in self._init_sim
        ...
        if observation_type == ObservationType.STATE:
            ...
            observations_size = (
                1 + 5 + 7 + 3 + num_torus_tracking_points * 3 + num_active_pegs * 2
            )
            self.observation_space = spaces.Box(
                low=-np.inf, high=np.inf, shape=(observations_size,), dtype=np.float32
            )

        # Image observations
        elif observation_type == ObservationType.RGB:
            self.observation_space = spaces.Box(
                low=0, high=255, shape=image_shape + (3,), dtype=np.uint8
            )

        elif observation_type == ObservationType.RGBD:
            self.observation_space = spaces.Box(
                low=0, high=255, shape=image_shape + (4,), dtype=np.uint8
            )

        else:
            raise ValueError(
                f"Please set observation_type to a value of ObservationType. Received {observation_type}."
            )
        ...
        if num_active_pegs < 1:
            raise ValueError(
                f"num_active_pegs must be greater than 0. Received {num_active_pegs}."
            )
        ...
        self.reward_amount_dict = defaultdict(dict) | {
            phase: defaultdict(float, **reward_dict)
            for phase, reward_dict in reward_amount_dict.items()
        }

        # Callback functions called on reset
        self.on_reset_callbacks = (
            on_reset_callbacks if on_reset_callbacks is not None else []
        )
    ...
    def _init_sim(self) -> None:
        ...
        self.target_positions: np.ndarray = self.scene_creation_result[
            "target_positions"
        ]
        ...
        self._distance_normalization_factor = 1.0 / np.linalg.norm(
            self.gripper.cartesian_workspace["high"]
            - self.gripper.cartesian_workspace["low"]
        )
        ...
        if self.num_torus_tracking_points > 0:
            if self.torus.num_points < self.num_torus_tracking_points:
                raise ValueError(
                    f"The number of torus tracking points ({self.num_torus_tracking_points}) is larger than the number of points on the torus ({self.torus.num_points})."
                )
            self.torus_tracking_point_indices = np.linspace(
                0,
                self.torus.num_points,
                num=self.num_torus_tracking_points,
                endpoint=False,
                dtype=np.int16,
            )
        elif self.num_torus_tracking_points == -1:
            self.torus_tracking_point_indices = np.array(
                range(self.torus.num_points), dtype=np.int16
            )
        elif self.num_torus_tracking_points == 0:
            self.torus_tracking_point_indices = None
        else:
            raise ValueError(
                f"num_torus_tracking_points must be > 0 or == -1 (to use them all). Received {self.num_torus_tracking_points}."
            )
    ...
    def _do_action(self, action: Union[int, np.ndarray]) -> None:
        ...
        self.gripper.set_articulated_state(
            self.gripper.get_articulated_state() + self._scale_action(action)
        )
    ...
    def _get_reward_features(self, previous_reward_features) -> dict:
        ...
        reward_features["established_grasp"] = (
            self.gripper.grasp_established
            and not previous_reward_features["grasped_torus"]
        )

        # Was there a grasp lost in this step?
        reward_features["lost_grasp"] = (
            previous_reward_features["grasped_torus"]
            and not self.gripper.grasp_established
        )

        # Distances between torus and the active pegs
        reward_features["torus_distance_to_active_pegs"] = np.linalg.norm(
            torus_center_of_mass - self.active_target_positions, axis=1
        )

        # Change in distances between torus and the active pegs
        reward_features["delta_torus_distance_to_active_pegs"] = (
            reward_features["torus_distance_to_active_pegs"]
            - previous_reward_features["torus_distance_to_active_pegs"]
        )

        # Distance between gripper and torus
        reward_features["gripper_distance_to_torus_center"] = np.linalg.norm(
            gripper_grasp_center_position - torus_center_of_mass
        )

        # Change in distance between gripper and torus
        reward_features["delta_gripper_distance_to_torus_center"] = (
            reward_features["gripper_distance_to_torus_center"]
            - previous_reward_features["gripper_distance_to_torus_center"]
        )
        ...
        if self.torus_tracking_point_indices is not None:
            ...
            reward_features[
                ...
                gripper_grasp_center_position
                - torus_positions[self.torus_tracking_point_indices],
                axis=1,
            )
            # Change in distance between gripper and torus tracking points
            reward_features["delta_gripper_distance_to_torus_tracking_points"] = (
                reward_features["gripper_distance_to_torus_tracking_points"]
                - previous_reward_features["gripper_distance_to_torus_tracking_points"]
            )
        else:
        ...
        reward_features["gripper_jaw_peg_collisions"] = sum(
            [listener.getNumberOfContacts() for listener in self.contact_listeners]
        )
        reward_features["gripper_jaw_floor_collisions"] = (
            self.gripper.get_grasp_center_position()[1] < 0.0
        )
        ...
        ) / (self.time_step * self.frame_skip)
        torus_probably_unstable = (
            mean_torus_velocity > 1.5 * gripper_velocity and mean_torus_velocity > 25.0
        )
        self.previous_gripper_position[:] = gripper_position
        ...
        # peg radius: 5.0 mm
        peg_in_torus_xz = np.any(
            np.linalg.norm(
                self.active_target_positions[:, [0, 2]] - torus_center_of_mass[[0, 2]],
                axis=1,
            )
            < 7.0
        )
        ...
        if self.block_done_when_torus_unstable:
            reward_features["successful_task"] = (
                peg_in_torus_xz and peg_in_torus_y and not torus_probably_unstable
            )
        else:
            reward_features["successful_task"] = peg_in_torus_y and peg_in_torus_xz

        # Did the torus drop off the board?
        reward_features["torus_dropped_off_board"] = np.mean(
            torus_positions[:, 1]
        ) < 3.0 and (
            np.abs(np.mean(torus_positions[:, 0])) > 80.0
            or abs(np.mean(torus_positions[:, 2])) > 80.0
        )

        # Did the action violate state or workspace limits?
        reward_features[
            "action_violated_cartesian_workspace"
        ] = self.gripper.last_set_state_violated_workspace_limits
        reward_features[
            "action_violated_state_limits"
        ] = self.gripper.last_set_state_violated_state_limits
        ...
        if reward_features["grasped_torus"]:
            reward_features["distance_to_minimum_pick_height"] = max(
                self.minimum_lift_height - torus_center_of_mass[1], 0.0
            )
            if "distance_to_minimum_pick_height" in previous_reward_features:
                reward_features["delta_distance_to_minimum_pick_height"] = (
                    reward_features["distance_to_minimum_pick_height"]
                    - previous_reward_features["distance_to_minimum_pick_height"]
                )
    ...
    def _get_reward(self) -> float:
        ...
        if self.active_phase == Phase.PICK:
            if (
                reward_features["grasped_torus"]
                and max(
                    self.minimum_lift_height - self.torus.get_center_of_mass()[1], 0.0
                )
                <= 0.0
            ):
        ...
        reward_amount_dict = (
            self.reward_amount_dict[Phase.ANY]
            | self.reward_amount_dict[self.active_phase]
        )

        # These values are arrays -> reduce them to a scalar
        if self.torus_tracking_point_indices is not None:
            reward_features["gripper_distance_to_torus_tracking_points"] = np.min(
                reward_features["gripper_distance_to_torus_tracking_points"]
            )
            reward_features["delta_gripper_distance_to_torus_tracking_points"] = np.min(
                reward_features["delta_gripper_distance_to_torus_tracking_points"]
            )
        if self.num_active_pegs > 1:
            reward_features["torus_distance_to_active_pegs"] = np.min(
                reward_features["torus_distance_to_active_pegs"]
            )
            reward_features["delta_torus_distance_to_active_pegs"] = np.min(
                reward_features["delta_torus_distance_to_active_pegs"]
            )
        ...
        for key, value in reward_features.items():
            ...
            if "distance" in key or "velocity" in key:
                ...
                value = np.clip(
                    value,
                    -1.0 / self._distance_normalization_factor,
                    1.0 / self._distance_normalization_factor,
                )
    ...
    def _get_observation(
        self, maybe_rgb_observation: Union[np.ndarray, None]
        ...
        if self.observation_type == ObservationType.RGB:
            ...
            state_dict = {}
            state_dict["has_grasped"] = np.array(float(self.gripper.grasp_established))[
                None
            ]  # make it a 1D array that can be concatenated
            state_dict["ptsda_state"] = self.gripper.get_articulated_state()
            state_dict["gripper_pose"] = self.gripper.get_pose()
            state_dict[
                "torus_center_of_mass_position"
            ] = self.torus.get_center_of_mass()
            if self.torus_tracking_point_indices is not None:
                state_dict[
                    "torus_tracking_point_positions"
                ] = self.torus.get_positions()[
                    self.torus_tracking_point_indices
                ].ravel()
            state_dict["active_peg_positions"] = self.active_target_positions[
                :, [0, 2]
            ].ravel()
    ...
    def _get_info(self) -> dict:
        ...
        if "distance_to_minimum_pick_height" not in self.reward_features:
            self.reward_features["distance_to_minimum_pick_height"] = (
                self.minimum_lift_height - self.torus.get_center_of_mass()[1]
            )
    ...
    def reset(
        ...
        if self._initialized:
            ...
            translation_offset = (
                new_position
                - self.pegs[0].mechanical_object.position.array()[:, :3]
            )
