...
class ColorEmbedder(MessagePassing):
    def __init__(
        self,
        mlp_1: MLP,
        mlp_2: MLP,
    ...
    def forward(
        self, pos: Tensor, batch: Tensor, color: Tensor
        ...
        edges = torch.stack([to_idx, from_idx], dim=0)
        try:
            x, neighborhoods = self.propagate(
                edges, pos=(pos, center_points), x=(color, color[center_points_idx])
            )
        except RuntimeError as e:
            torch.set_printoptions(threshold=sys.maxsize)
            logger.log(
